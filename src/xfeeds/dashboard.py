...
.orcid-id{display:inline-flex;align-items:center;gap:.42em;vertical-align:baseline}
.orcid-mark{width:1.05em;height:1.05em;flex:none;position:relative;top:.14em}
...
def _status(
    ...
    # Acknowledged-expired sources (ADR-059) are deliberately dormant, not
    # failing. They are excluded from the active denominator and from the
    # "needs review" list; they are returned separately so the strip can show
    # them as acknowledged rather than as missing.
    expired = [
    ...
    bad = [
        (name, str(info.get("status", "unknown")))
        for name, info in entries
        if info.get("status") != "ok" and info.get("status") != "expired"
    ]
    active_total = len(entries) - len(expired)
    ok = active_total - len(bad)
    return ok, active_total, bad, expired
...
def _status_strip(manifest: dict[str, Any]) -> str:
    ok, total, bad, expired = _status(manifest)
    ...
    dormant = ""
    if expired:
        names = ", ".join(name for name, _ in expired)
        dormant = f'<span class="divider optional">·</span><span class="dormant-link optional">{len(expired)} dormant: {esc_html(names)} (acknowledged)</span>'
    return f"""<div class="status-strip"><div class="status-inner"><span class="pulse" aria-hidden="true">●</span><strong>Feed live</strong><span class="divider">·</span><span>rebuilt <time datetime="{esc_html(machine)}">{esc_html(clock)} UTC</time></span><span class="divider cadence">·</span><span class="cadence">every 6 hours</span><span class="divider optional">·</span><span class="optional"><strong>{ok}/{total}</strong> active sources OK</span>{issue}{dormant}</div></div>"""
...
def _availability(history: list[dict[str, Any]]) -> str:
    ...
    for number, item in enumerate(points, start=1):
        ...
        expired_count = _int(item, "sources_expired")
        height = 6 + round(18 * published / peak)
        label = (
            f"Run {number}: {published:,} published, {source_ok} of {source_total} active sources healthy"
            + (f", {expired_count} dormant" if expired_count else "")
        )
        ticks.append(
    ...
    return (
        ...
        '<span class="run-key">'
        '<span><i class="k-ok"></i>All active sources OK</span>'
...
def _hero_history_chart(history: list[dict[str, Any]]) -> str:
    ...
    for index, item in enumerate(runs):
        ...
        summary = (
            ...
            f"{_int(item, 'sources_ok')}/{_int(item, 'sources_total')} active sources"
        )
...
def _about_section(manifest: dict[str, Any]) -> str:
    ...
    return (
        ...
        "<p>Maintained by Neil Weitzel<br>"
        f"{_orcid_link()}</p>"
        "<p>Machine-readable citation metadata is in "
...
def _source_rows(manifest: dict[str, Any]) -> str:
    ...
    for name, info in sorted(sources.items()):
        ...
        # Expired is not a louder kind of stale, it is a different outcome, so it
        # does not share stale's styling (ADR-059).
        if status == "ok":
        ...
        # "Publication" answers what this source puts into the feed, which is not
        # the same question as whether its licence would allow it to. An expired
        # source is usually still redistributable on paper - Feodo Tracker is CC0 -
        # and contributes nothing, so reading the licence flag alone printed "yes"
        # next to a source supplying zero records.
        if status == "expired":
            republished = "not contributing"
        elif bool(info.get("redistributable")):
            republished = "yes"
        else:
            republished = "scoring only"
        rows.append(
...
def render_analysis(
    ...
    networks = insights.get("networks", {})
    _, _, bad, expired = _status(manifest)
    if bad:
        status_detail = ", ".join(f"{name} ({state})" for name, state in bad)
        if expired:
            status_detail += f"; {len(expired)} dormant source{'s' if len(expired) != 1 else ''} acknowledged (visible in the source table)."
        else:
            status_detail += "."
    elif expired:
        status_detail = (
            f"All active sources reported cleanly; {len(expired)} dormant source"
            f"{'s' if len(expired) != 1 else ''} remains visible in the source table."
        )
    else:
        status_detail = "No source needs review."
    nav = "".join(
